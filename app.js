...
onAuthStateChanged(auth, user => {
  ...
  if(user){
    ...
    if(document.getElementById("authStatus")) authStatus.textContent = user.displayName || user.email;
    if(document.getElementById("sideProfileName")) sideProfileName.textContent = user.displayName || user.email;
    if(document.getElementById("sideProfileSub")) sideProfileSub.textContent = "Signed in member";
    if(document.getElementById("authButton")) authButton.classList.add("hidden");
    if(document.getElementById("logoutButton")) logoutButton.classList.remove("hidden");
    const nameInput = document.getElementById("postName");
    if(nameInput) nameInput.value = user.displayName || user.email;
  }else{
    if(document.getElementById("authStatus")) authStatus.textContent = "Guest";
    if(document.getElementById("sideProfileName")) sideProfileName.textContent = "USArab Member";
    if(document.getElementById("sideProfileSub")) sideProfileSub.textContent = "Sign in / create profile";
    if(document.getElementById("authButton")) authButton.classList.remove("hidden");
    if(document.getElementById("logoutButton")) logoutButton.classList.add("hidden");
  }
});
...
function renderRightMembers(){
  const el = document.getElementById("rightMembers");
  if(!el) return;
  el.innerHTML = users.slice(0,5).map(u => `<div class="suggest-row"><div class="suggest-icon">${(u.name || u.email || "M").charAt(0).toUpperCase()}</div><div><b>${u.name || "Member"}</b><br><small>${u.role || "member"}</small></div></div>`).join("") || "<p>No members yet.</p>";
}
