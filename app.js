...
function renderMembers(){
  ...
  const filtered = users.filter(u => {
    const text = `${u.name || ""} ${u.email || ""} ${u.city || ""} ${u.role || ""}`.toLowerCase();
    return !q || text.includes(q);
  });

  container.innerHTML = filtered.map(u => {
    const status = friendshipStatus(u.uid || u.id);
    let button = "";
    if(status === "guest") button = `<button onclick="openAuthModal()">Sign in to Add</button>`;
    else if(status === "self") button = `<button class="secondary">This is You</button>`;
    else if(status === "none") button = `<button onclick="sendFriendRequest('${u.uid || u.id}', '${(u.name || u.email || "Member").replace(/'/g,"")}')">Add Friend</button>`;
    else if(status === "sent") button = `<button class="secondary">Request Sent</button>`;
    else if(status === "received") button = `<button onclick="acceptFriendByUser('${u.uid || u.id}')">Accept Request</button>`;
    ...
    return `<article class="member-card searchable" data-search="${`${u.name || ""} ${u.email || ""} ${u.city || ""} ${u.role || ""}`.toLowerCase()}">
      ...
      <span class="friend-status">${status === "friends" ? "Friends" : status === "sent" ? "Request Sent" : status === "received" ? "Request Received" : "Member"}</span>
      <div>${button}</div>
    </article>`;
